                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      Text(
                        'Username: ${order.account?.accountUsername ?? "Unknown"}',
                        style: TextStyle(fontSize: 16),
                      ),
                      SizedBox(height: 8),
                      Text(
                        'Date: ${order.orderCreatedAt?.toString().substring(0, 10) ?? "N/A"}',
                        style: TextStyle(fontSize: 16),
                      ),
                      SizedBox(height: 8),
                      Text(
                      ...
                      SizedBox(height: 8),
                      Text(
                        'Total Price: \$${order.orderTotalPrice?.toStringAsFixed(2) ?? "0.00"}',
                        style: TextStyle(fontSize: 16),
                      ),
                      SizedBox(height: 8),
                      Text(
                        'Address: ${order.account?.address ?? "N/A"}',
                        style: TextStyle(fontSize: 16),
                      ),
                      SizedBox(height: 8),
                      Text(
                        'Phone: ${order.account?.phoneNumber ?? "N/A"}',
                        style: TextStyle(fontSize: 16),
                      ),
                      SizedBox(height: 8),
                      if (order.orderStatus == 'Pending')
                        ElevatedButton(
                          onPressed: () async {
                            try {
                              await AdminOrderService.approveOrder(
                                order.orderId,
                              );
              ...
              ...(order.tblOrderItems ?? [])
                  .map(
                    (item) => Card(
                      ...
                      child: ListTile(
                        title: Text(
                          item.dish?.dishName ?? 'Unknown',
                          style: TextStyle(fontWeight: FontWeight.bold),
                        ),
                        subtitle: Text(
                          'Quantity: ${item.orderItemQuantity ?? 0} | Price: \$${item.orderItemPrice?.toStringAsFixed(2) ?? "0.00"}',
                        ),
                      ),
                    ),
                  )
